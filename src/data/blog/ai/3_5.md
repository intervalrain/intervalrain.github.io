...
author: "Rain Hu"
pubDatetime: 2024-12-19T15:55:12+08:00
description: "The introduction to logistic regression"
tags: [logistic-regression, bayes-theorem, gaussian-distribution, binary-classification, sigmoid]
---
...
+ 假設有一個抽獎箱內有紅球與藍球，球上有標示 A 與 B 類別。
    ...
    + 我們抽到來自 A 的藍球的機率為 $P(A|\textcolor{blue}{\bullet})$
    ...
    2. 條件機率
        + $P(\textcolor{blue}{\bullet}|A)=\frac{\text{A中的}\textcolor{blue}{\bullet}}{\text{A的總球數}}=\frac{3}{4}$
        + $P(\textcolor{blue}{\bullet}|B)=\frac{\text{B中的}\textcolor{blue}{\bullet}}{\text{B的總球數}}=\frac{2}{6}$
    套入公式可得

    $$
    P(A|\textcolor{blue}{\bullet})=\frac{P(\textcolor{blue}{\bullet}|A)P(A)}{P(\textcolor{blue}{\bullet}|A)P(A)+P(\textcolor{blue}{\bullet}|B)P(B)}=\frac{3/4\times4/10}{3/4\times4/10+2/6\times6/10}=\frac{3}{5}
    $$
...
+ 我們嘗試使用高斯分布模型對一個簡單的問題進行描述：
    ...
    def generate_samples(self, n_samples=1000):
        ...
        return data, genders
    ```
    ![distribution](/ai/3_5/distribution.png)
    + **假設**
        + $x = [\text{身高}, \text{體重}]^T \in \mathbb{R}^2$：每個數據點的特徵向量。
        + 類別 $y \in \{0, 1\}$：性別標籤，0 表示女，1 表示男。
    ...
    + 後面的這一項，可以從訓練資料得到 $\frac{P(A)}{P(B)}=\frac{N_A}{N_B}$
        ...
        $$
        z=(\mu_A-\mu_B)^T\Sigma^{-1}\textcolor{red}{x}-\frac{1}{2}(\mu_A)^T(\Sigma_A)^{-1}\mu_A+\frac{1}{2}(\mu_B)^T(\Sigma_B)^{-1}\mu_B+\ln\frac{N_A}{N_B}
        $$
...
$$
+ $\hat{y}_i=0$ 或 $1$，在 $f(x_i)=0$ 或 $1$ 時，$\frac{\partial L}{\partial w_i}=0$，換言之在接近 target，與相當遠離 target 的地方，梯度都為 0，也就是說使用 MSE 會有梯度遺失的問題。
